...
static int music_artist_cmp(const void *m1, const void *m2);
static int music_playlist_print(FILE *pf, const void *m);
static Status mergeStacks(Stack *sin1, Stack *sin2, Stack *sout, P_stack_ele_cmp fcmp);
...
static int music_artist_cmp(const void *m1, const void *m2) {
  ...
  cmp = strcmp(music_getArtist(music1), music_getArtist(music2));
  if (cmp != 0) return cmp;

  return music_cmp(music1, music2);
}
...
static Status mergeStacks(Stack *sin1, Stack *sin2, Stack *sout, P_stack_ele_cmp fcmp) {
  ...
  if (!sin1 || !sin2 || !sout || !fcmp) return ERROR;

  while (stack_isEmpty(sin1) == FALSE && stack_isEmpty(sin2) == FALSE) {
    if (fcmp(stack_top(sin1), stack_top(sin2)) > 0) {
      e = stack_pop(sin1);
    } else {
      e = stack_pop(sin2);
    }
...
int main(int argc, char **argv) {
  ...
  if (mergeStacks(playlist1, playlist2, merged, music_artist_cmp) == ERROR) {
